...
В текущей сборке также подтверждены: полный снимок Display 1 размером 2880×1800, preview, одноразовое вложение снимка к следующему вопросу и его очистка после отправки; ввод вопроса и завершение потокового Fake-ответа; запуск системного TTS с появлением pause/stop и немедленная остановка; сохранение выбранной светлой темы после перезапуска и возврат к системной теме. На старте и в покое процесс не имел TCP/UDP-сокетов. Статусный пункт меню создан кодом и собирается, но его отдельная визуальная runtime-проверка в строке меню ещё открыта.
